...
def _merge_a_into_b(a, b):
  """Merge configs dictionary a into configs dictionary b, clobbering the
  ...
  for k, v in a.items():
    ...
    if k not in b:
      raise KeyError('{} is not a valid configs key'.format(k))
    ...
    if old_type is not type(v):
      if isinstance(b[k], np.ndarray):
        ...
        raise ValueError(('Type mismatch ({} vs. {}) '
                          'for configs key: {}').format(type(b[k]),
                                                       type(v), k))
    ...
    if type(v) is edict:
      try:
        ...
        print(('Error under configs key: {}'.format(k)))
        raise
    else:
      b[k] = v
...
def cfg_from_file(filename):
  """Load a configs file and merge it into the default options."""
  import yaml
